...
def test_temporal_split_uses_latest_trainable_year(sample_data):
    feature_df = create_features(sample_data).dropna(subset=["target_next"])
    train, test, year = temporal_split(feature_df)
...
def test_ranking_is_sorted(sample_data):
    feature_df = create_features(sample_data)
    model, _, _ = train_and_compare(feature_df)
    ranking = build_ranking(model, feature_df)
